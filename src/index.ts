...
function setUpRepositoryOptions(args: Argv<object>) {
    args
        ...
        .option('repository', {
            alias: 'r',
            choices: repositoryChoices.map(repo => repo.name),
            type: 'string',
        ...
        .option('name', {
            alias: 'n',
            type: 'string',
            description: 'Name of the repository that will be generated. By default the application generates (entityname)Repository.class'
        })
}

yargs
    .version("1.0")
    .command(['*', "interactive", "i"], 'Load the Interactive mode', {}, handleInteractiveMode)
